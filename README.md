...
index.html, style.css and app.js are the dashboard itself: one scrolling page with the summary cards, the charts, the hover world map, and a searchable table. The CDN chart is stacked: each provider's bar splits into the sites already negotiating PQC and the ones not, so it also reads as that provider's PQC readiness. There is also an adoption-over-time line (one point per scan on file), and clicking any row in the site table opens a one-sentence "next step" for that site, worked out from what the scan already knows about it and its provider. The chart and map libraries load from a CDN, so there is nothing to build or install.

The data folder holds sites.csv (the list of sites, each with a sector and country) and one scan file per date. There is also sites-ca-toplist.csv, a separate list of the sites Canadians visit most. Since Jul 20 that list follows a published source instead of being hand-picked: Similarweb's "Top Websites in Canada" ranking (https://www.similarweb.com/top-websites/canada/, refreshed monthly), kept in rank order, with adult sites excluded. That gives the "most visited by Canadians" question a consistent, citable definition. The list is kept apart from sites.csv on purpose, so those global sites do not get counted twice on the world map.

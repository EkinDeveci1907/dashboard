...
The data folder holds sites.csv (the list of sites, each with a sector and country) and one scan file per date. There is also sites-ca-toplist.csv, a separate list of the sites Canadians visit most. I used to hand-pick that list; now it is just Similarweb's published Top Websites in Canada ranking (similarweb.com/top-websites/canada, refreshed monthly) in rank order, with the adult sites dropped. That way, when someone asks what "most visited by Canadians" means, the answer is a source and not my judgement. The list is kept apart from sites.csv on purpose, so those global sites do not get counted twice on the world map.
